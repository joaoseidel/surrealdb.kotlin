...
private fun row(body: String): Row = Row.fromJson(Json, Json.parseToJsonElement(body) as JsonObject)
    ...
    ShouldSpec(
        {
            ...
            context("a field whose type is not known at the call site") {
                should("decode through a serializer passed in, because reified refuses a Field<*>") {
                    val projection: List<com.surrealdb.kotlin.query.api.data.Field<*>> = listOf(Sightings.name)
