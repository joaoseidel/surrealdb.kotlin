...
private fun <T> TestScope.collectInto(flow: Flow<T>): MutableList<T> {
    ...
    // race. An unconfined one runs to the first suspension, so the subscription
    // is active before this returns.
    ...
    ShouldSpec(
        {
            context("LiveNotificationRouter") {
                ...
                context("a registered subscription") {
                    ...
                    should(
                        "end when the same id is registered again, because notifications now go to the replacement",
                    ) {
                        runTest {
                            val router = LiveNotificationRouter()
                ...
                context("activeQueries") {
                    ...
                    should(
                        "list a query tracked without a channel, since a collector filtering the broadcast opens none",
                    ) {
                        runTest {
                            val router = LiveNotificationRouter()
